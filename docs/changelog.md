...
- All Files view in File Explorer for users holding one of those permissions, loaded page by page from the server with search and sorting.
...
- File Explorer loads long file lists progressively as the list is scrolled, instead of rendering every row at once, so the tab stays responsive in organizations with a large number of files.
